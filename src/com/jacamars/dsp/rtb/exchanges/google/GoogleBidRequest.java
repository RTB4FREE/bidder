	...
	void makeSiteOrApp() throws Exception {
		...
		if (internal.hasSite()) {
			...
			if (a.hasContent()) {
				...
				if (c.hasContentrating()) content.put("contentrating", c.getContentrating());
	...
	void makeImpressions() {
		...
		for (int i=0;i<internal.getImpCount();i++) {
			...
			if (imp.getExtension(AdxExt.imp).getBillingIdCount() > 0) {
				...
				ext.set("blid", getAsBillingIdList(BidRequest.factory.arrayNode(), imp.getExtension(AdxExt.imp).getBillingIdList()));
				impx.set("ext", ext);
			}
